...
blocks_config_dir = get_resource_path('blocks/configs')
block_list = os.listdir(blocks_config_dir)
all_blocks = {}
for filename in block_list:
    if filename.endswith('.yaml'):
        with open(os.path.join(blocks_config_dir, filename), 'r', encoding='utf-8') as file:
...
class Block:
    ...
    def caption(self, finished=False, prefix: str = None):
        ...
        message = []
        if self.note:
            message.append(f'({self.note}) [bold cyan]<ID {prefix}_{self.local_id}: {self.name}>[/bold cyan]' if prefix else f'({self.note}) [bold cyan]<ID {self.local_id}: {self.name}>[/bold cyan]')
        else:
            message.append(f'[bold cyan]<ID {prefix}_{self.local_id}: {self.name}>[/bold cyan]' if prefix else f'[bold cyan]<ID {self.local_id}: {self.name}>[/bold cyan]')
...
Returns:
    str: Status message about the attachment operation
        ...
        if not self.started and not self.assembly:
            error_message = "[bold red]The machine has not been started, please start the machine first.[/bold red]"
            self.update_prompt(pre_msg=error_message)
            self.log_failed_operation("attach_block_to", error_message)
            return self.prompt  
        if new_block in AvailableConnectors:
            error_message = "[bold red]Connectors can not be attached to a single face. Use 'connect_blocks' to connect two faces instead.[/bold red]"
            self.update_prompt(pre_msg=error_message)
            self.log_failed_operation("attach_block_to", error_message)
            return self.prompt
        if new_block not in AvailableBlocks and new_block not in WaiveableBlocks:
            error_message = f"[bold red]Block {new_block} not available, please try again.[/bold red]"
            self.update_prompt(pre_msg=error_message)
            self.log_failed_operation("attach_block_to", error_message)
            return self.prompt
        else:
            ...
            if base_block in self.blocks.keys():
                ...
                if face in base_block.faces.keys() and base_block.faces.get(face).sticky:
                    ...
                    error_message = f"[bold red]Base block {base_block.local_id} {base_block.name} does not have face {face}, please try again.[/bold red]"
                    self.update_prompt(pre_msg=error_message)
                    self.log_failed_operation("attach_block_to", error_message)
                else:
                    error_message = f"[bold red]Face {face} of base block {base_block.local_id} {base_block.name} is already occupied, please try again.[/bold red]"
                    self.update_prompt(pre_msg=error_message)
                    self.log_failed_operation("attach_block_to", error_message)
            else:
                error_message = f"[bold red]Base block {base_block} not found, please try again.[/bold red]"
                self.update_prompt(pre_msg=error_message)
                self.log_failed_operation("attach_block_to", error_message)
...
Returns:
    str: Status message about the twist operation
        ...
        if block_id in self.blocks.keys():
            ...
            error_message = f"[bold red]Specified block {block_id} not found, please try again.[/bold red]"
            self.update_prompt(pre_msg=error_message)
            self.log_failed_operation("twist_block", error_message)
        return self.prompt
...
Returns:
    str: Status message about the flip operation
        ...
        if block_id in self.blocks.keys():
            ...
            if block.spin:
                ...
                error_message = f'[bold red]The block {block_id} <{block.name}> does not spin, please try again.[/bold red]'
                self.update_prompt(pre_msg=error_message)
    ...
    def collision_detect(self, target_block_id = False, assembly = False):
        ...
        if is_collision:
            ...
            collision_msg = [f"[bold red]Operation failed! The {block_label} overlaps with existing blocks and it's been restored to previous state already, please try again[/bold red]"]
            for i, j in iter(collision_pairs):
                collision_msg.append(f"[bold red]Overlapping detected between <ID {self.blocks[i].local_id}: {self.blocks[i].name}> and <ID {self.blocks[j].local_id}: {self.blocks[j].name}>[/bold red]")
...
Returns:
    ...
    # TODO: expose certain blocks as anchor points before saving
    def to_file(self, output_dir, shift_virtual: List[float] = [0, 0, 0], rotation: List[float] = [0, 0, 0, 1]):
        ...
        self.output_file_path = os.path.join(output_dir, f'{self.name}.bsg')
        self.output_sequence_path = os.path.join(output_dir, f'{self.name}.json')
        self.full_op_history_path = os.path.join(output_dir, f'{self.name}_full.json')
